...
for i in range(0,experiments):
    for j in range(0, experiments):
        ...
        LeNet1_Tanh2.to(device)
        train_net(LeNet1_Tanh2, i+1, device)
        acc = test_net(LeNet1_Tanh2, device)
        accuracies.append(acc)
...
for i in range(0,experiments):
    for j in range(0, experiments):
        ...
        LeNet1_SP2.to(device)
        train_net(LeNet1_SP2, i+1, device)
        acc = test_net(LeNet1_SP2, device)
        accuracies.append(acc)
...
for i in range(0,experiments):
    for j in range(0, experiments):
        LeNet1_TayloyEx4 = nn.Sequential(
            nn.Conv2d(1, 4, kernel_size=5),
            TaylorExpansion4(),
            nn.AvgPool2d(kernel_size=2),
        ...
        LeNet1_TayloyEx4.to(device)
        train_net(LeNet1_TayloyEx4, i+1, device)
        acc = test_net(LeNet1_TayloyEx4, device)
        accuracies.append(acc)
