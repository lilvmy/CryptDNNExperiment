...
def relu(HE, image):
    try:
        decrypt_image = decrypt_matrix(HE, image)
        relu_image = np.array(np.maximum(decrypt_image, 0.0))
        return np.array(list(map(lambda x: HE.encryptFrac(x), relu_image)))

    except TypeError:
        return np.array([relu(HE, m) for m in image])
